...
public struct HealthSystem
{
    ...
    public void TakeDamage(FleetManager _selfFleetManager, List<DataShip> locEnemyDataShips)
    {
        ...
        for (int i = 0; i < locEnemyDataShips.Count; i++)
        {
            ...
            //Debug.Log($"ќбщий урон {locTotalDamage}");

            //јтакующей флот стрел€ет. ‘лот по которому попали обрабатывает попадание. 
            while (locTotalDamage > 0 && locSelfListShips.Count > 0)
            {
                ...
                int targetIndex = random.Next(0, locSelfListShips.Count); // ¬ыбираем случайный корабль дл€ нанемени€ урона
                ...
                //Debug.Log($"≈сть ли увеличенный урон? {increasedDamage != 1f}");

                float damageToApply =
                    Math.Min(targetShip.armorShip + targetShip.shieldShip, ( locTotalDamage * increasedDamage)); // ¬ычисл€ем урон

                //Debug.Log($" ƒо <color=red> shield {targetShip.shieldShip}  armor {targetShip.armorShip} </color>");
                ...
                //Debug.Log($"<color=green> ѕосле shield {targetShip.shieldShip}  armor {targetShip.armorShip} </color>");
